package com.example.ptweb.repository.statistics;


import com.example.ptweb.Service.statistics.AggregatedStatistics;
...
public interface StatisticsRepository extends JpaRepository<StatisticsEntity , Integer> {
    @Query(value = "SELECT new com.example.ptweb.Service.statistics.AggregatedStatistics(s.statisticsAt, SUM(s.allCount), SUM(s.attendedCount), SUM(s.cancelledCount)) " +
